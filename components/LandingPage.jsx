...
//       style={{ backgroundImage: "url('/slider/agro1.jpg')" }}
//     >
//       {/* Overlay */}
...
const LandingPage = () => {
  const { t } = useTranslation("common"); // ✅ always call hooks first
  const hasMounted = useHasMounted(); // ✅ call second
  const [activeIndex, setActiveIndex] = useState(0);
  const sliderRef = useRef(null);
  const textRefs = useRef([]);
  ...
  useEffect(() => {
    if (!hasMounted) return; // ✅ guard inside effect, not before hooks
    const currentText = textRefs.current[activeIndex];
    if (currentText) {
      gsap.fromTo(
        currentText,
  ...
  if (!hasMounted) {
    // ✅ move conditional render *after* all hooks
    return <div className="w-full h-[80vh] bg-black" />; // placeholder to keep SSR markup stable
  }
